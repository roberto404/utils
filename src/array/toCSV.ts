 ...
 * @return {string}       csv
 */
const toCSV = (items: Array<any>, hook?: {}): string =>
{
  const columns = hook ||
  ...
  return header + items
    .map(item => Object
      .keys(columns)
      .map(column => ENCLOSER + (hook && hook[column] && typeof hook[column].format === 'function' ? hook[column].format(item) : (item[column] || '')) + ENCLOSER)
      .join(DELIMITER))
